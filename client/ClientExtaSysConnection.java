 ...
 */
public class ClientExtaSysConnection extends Thread implements ActionListener, Constants, ClientConnection
{
    // SINGLETONS
     ...
     * @param engine
     */
    public ClientExtaSysConnection(GameEngine engine)
    {
        try
        {
            this.engine = engine;
    ...
    public int loginToServer(String serverName, String userName) throws Exception
    {
         ...
         * buffer.array(); // Check if we successfully logged in
         ...
         * %d\n", port); // create datagram channel & connect to rem port
         ...
         * sockChannel.write(ByteBuffer.wrap(loginSuccess)); // Get your
         * multicast address and port myMCastGroup =
         ...
         * mSocket.joinGroup(myMCastGroup); // Create a pipe to write back to
         * the connection pipe = Pipe.open(); source = pipe.source();
         * 
         * source.configureBlocking(false);
         * source.register(selector,SelectionKey.OP_READ); // Notify everyone
         * that we have joined PlayerJoinMessage message = new
