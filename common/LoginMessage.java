...
public class LoginMessage
{
	...
	public static byte[] getLoginMessage(String username, String passwd)
	{
		...
		Charset charset = Charset.forName(CHARSET_NAME);
		String message = new String("LOGIN" + SEPERATOR + "NAME=" + username
				+ SEPERATOR + "PASS=" + passwd + SEPERATOR);
	...
	{
		Charset charset = Charset.forName(CHARSET_NAME);
		String message = new String("SUCCESS" + SEPERATOR + PORT_HEADER
				+ Integer.toString(port) + SEPERATOR);
		return charset.encode(message).array();
	}
	...
	{
		Charset charset = Charset.forName(CHARSET_NAME);
		String message = new String("FAILURE" + SEPERATOR + FAIL_REASON_HEADER
				+ reason + SEPERATOR);
		return charset.encode(message).array();
	}
	...
	{
		String message = getMessageString(messagedata);
		int start = message.indexOf(FAIL_REASON_HEADER)
				+ FAIL_REASON_HEADER.length();
		int end = message.indexOf(SEPERATOR, start);
		return message.substring(start, end);
	}
}
