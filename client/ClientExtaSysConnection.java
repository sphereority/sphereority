...
public class ClientExtaSysConnection extends ExtasysUDPClient implements IUDPClient, Constants, ClientConnection
{
    ...
    public void establishServerConnection() throws Exception
    {
        ...
        SendMessage(new PlayerJoinMessage((byte) -1, engine.localPlayer.getPlayerName(), serverAddress, sp, false));
     ...
     * 
     * @param connector
     *            The connector the message was received from.
     * @param packet
     *            The packet the message was sent from.
     */
    // @Override
    public void OnDataReceive(UDPConnector connector, DatagramPacket packet)
    {
        try
        {
            ...
            if (message.getPlayerId() == engine.localPlayer.getPlayerID() || message.getPlayerId() == -1)
                return;
            ...
            switch (message.getMessageType())
            {
                case PlayerMotion:
                    ...
                    logger.log(Level.INFO, "PlayerMotion: " + pm.getPlayerId() + " " + pm.getPosition() + " " + pm.getVelocity() + " " + pm.getTime());
                    break;
                case PlayerJoin:
                ...
                case Projectile:
                    ...
                    logger.log(Level.INFO, "Projectile: " + p.getPlayerId() + " " + p.getStartPosition() + " " + p.getDirection());
    ...
    /**
     * Add a new connector to this client that can be either unicast or
     * multicast.
     * 
     * @param name
     *            is the name of the connector.
     * @param readBufferSize
     *            is maximum number of bytes the connector can read at a time.
     * @param readTimeOut
     *            is the maximum time in milliseconds the connector can use to
     *            read incoming data.
     * @param serverIP
    ...
    public UDPConnector AddConnector(String name, int readBufferSize, int readTimeOut, InetAddress serverIP, int serverPort, boolean isMulticast)
    {
        ...
        connector = isMulticast ? new MulticastConnector(this, name, readBufferSize, readTimeOut, serverIP, serverPort) : new UDPConnector(this, name, readBufferSize, readTimeOut, serverIP, serverPort);
    ...
    public void run()
    {
        ...
        for (int checkNames = 0; fActive; checkNames++)
        {
            try
            {
                ...
                fMyClient.sendMessage(new PlayerMotionMessage((byte) localPlayer.getPlayerID(), localPlayer.getPosition(), localPlayer.getVelocity(), (float) System.currentTimeMillis()));
                ...
                for (common.Projectile p : engine.bulletList)
                {
                    ...
                    if (!p.isDelivered() && !(p.getOwner() != playerId))
                    {
                        ...
                        fMyClient.sendMessage(new ProjectileMessage(playerId, p.getStartPosition(), p.getDirection()));
                        p.delivered();
                    }
                }
                ...
                if (checkNames == 10)
                {
                    for (Player player : engine.playerList)
                    {
                        ...
                        {
                            fMyClient.sendMessage(new PlayerJoinMessage((byte) player.getPlayerID(), RESOLVING_NAME, new InetSocketAddress(SERVER_ADDRESS, SERVER_PORT), new SpawnPoint(player.getPosition())));
                        }
                    }
                }
