...
class Connection extends Thread
{
	...
	public Connection(String uname, ServerGameEngine sge, SocketChannel sc,
	...
	public void run()
	{
		...
		try
		{
			...
			ObjectOutputStream sockoutputstream = new ObjectOutputStream(
					sockchannel.socket().getOutputStream());
			...
			} else
			{
				...
				dsockchannel.socket().connect(
						new InetSocketAddress(sockchannel.socket()
								.getInetAddress(), udp_remoteport));
				...
				socket_key = sockchannel.register(selector,
						SelectionKey.OP_READ | SelectionKey.OP_WRITE);
				socket_key.attach(oistream);
				dsockchannel.configureBlocking(false);
				dgram_socket_key = dsockchannel.register(selector,
						SelectionKey.OP_READ | SelectionKey.OP_WRITE);
	...
	private void processSelectionKey(SelectionKey selKey)
	{
		if (selKey.isValid() && selKey.isReadable())
		{
			...
			/*
			 * if (selKey.channel() == sockchannel){ try { Message message =
			 * (Message) oistream.readObject();
